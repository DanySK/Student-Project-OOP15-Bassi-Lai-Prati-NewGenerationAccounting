...
public class FinancialSituationModelImpl implements IFinancialSituationModel {
	...
	public String getAnalisiFinanziaria() {
		 ...
		 * proprio (capitale acquisito dall’impresa o che permane nell’impresa a
		 * titolo di pieno rischio) copre tutto il fabbisogno durevole; se
		 * negativo significa che parte del fabbisogno è coperta da debiti.
		 *
		 * MARGINE DI TESORERIA MT = (LIQUIDITÀ DIFFERITE +LIQUIDITÀ IMMEDIATE)
		 * - PASSIVITÀ a BREVE Il margine di tesoreria dovrebbe essere positivo.
		 * Se il margine è negativo significa che l’impresa si trova in zona di
		 * tensione finanziaria a breve termine perché, di fronte ad una
		 * richiesta di rimborso immediato di tutti i debiti a breve termine,
		 * non avrebbe i mezzi finanziari necessari per farvi fronte.
		 *
		 ...
		 * margine è negativo significa che impieghi durevoli sono finanziati da
		 * passività a breve termine con riflessi preoccupanti sulla
		 * solvibilità.
		 *
		 * CAPITALE CIRCOLANTE NETTO OPERATIVO CCN O = ATTIVITA’ DELLA GESTIONE
		 * CARATTERISTICA CORRENTE - PASSIVITÀ DELLA GESTIONE CORRENTE Se il
		 * capitale circolante netto operativo è positivo significa che parte
		 * degli investimenti della gestione caratteristica corrente è
