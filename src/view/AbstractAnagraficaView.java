...
/**
 * classe astratta per le view che rispettano il layout di anagrafica, con i 5 tasti nel footer.
 * @author Pentolo
 *
 */
public abstract class AbstractAnagraficaView extends AbstractFrame {
	...
	public AbstractAnagraficaView(String title, AbstractAnagraficaViewObserver observer) {
		this(title, observer, DEFAULT_TASTO_0, DEFAULT_TASTO_1, DEFAULT_TASTO_2, DEFAULT_TASTO_3, DEFAULT_TASTO_4);
	}

	public AbstractAnagraficaView (String title, AbstractAnagraficaViewObserver observer, String testo0, String testo1, String testo2, String testo3, String testo4){
	...
	private void addListeners() {
		tasto0.addActionListener(e -> {((AbstractAnagraficaViewObserver) observer).tasto0();});
		tasto1.addActionListener(e -> {((AbstractAnagraficaViewObserver) observer).tasto1();});
		tasto2.addActionListener(e -> {((AbstractAnagraficaViewObserver) observer).tasto2();});
		tasto3.addActionListener(e -> {((AbstractAnagraficaViewObserver) observer).tasto3();});
		tasto4.addActionListener(e -> {((AbstractAnagraficaViewObserver) observer).tasto4();});
	}
}
